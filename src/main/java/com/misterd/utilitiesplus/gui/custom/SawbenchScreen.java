...
public class SawbenchScreen extends AbstractContainerScreen<SawbenchMenu> {
    ...
    public SawbenchScreen(SawbenchMenu menu, Inventory inventory, Component title) {
    ...
    private void extractButtons(GuiGraphicsExtractor graphics, int xm, int ym, int x, int y, int endIndex) {
        for (int index = this.startIndex; index < endIndex && index < this.menu.getNumberOfVisibleRecipes(); ++index) {
            ...
            int textureY = posY - 1;
            graphics.blitSprite(RenderPipelines.GUI_TEXTURED, recipeSprite, posX, textureY, 16, 18);
